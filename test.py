...
def main():
    with Ice.initialize(sys.argv) as communicator:
        server1 = "tcp -h 127.0.0.2 -p 10000 -z : udp -h 127.0.0.2 -p 10000 -z"
        server2 = "tcp -h 127.0.0.3 -p 10000 -z : udp -h 127.0.0.3 -p 10000 -z"
        for i, server in enumerate((server1, server2)):
            ...
            print(camera.moveDetected())
            fridge.put("CHEESE", 5)
            fridge.put("MILK", 2)
            print(fridge.list())
            print(central_heating.getTemperature())
            garage.open()
            garage.close()
            print(garage.isOpen())
            garden_camera.turnInfraredOn()
            garden_camera.turnInfraredOff()
            lamp_4.turnOff()
            lamp_4.turnOn()
            lamp_4.isOn()
